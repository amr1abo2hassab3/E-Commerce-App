...
export const CardBrand = ({ brand }) => {
  return (
    <Link to={`/brandDetails/${brand._id}`}>
      <div
        className={`bg-[#FEFEFE] flex  rounded cursor-pointer transition-all duration-500 hover:rotate-3 hover:scale-90  shadow-[0px_10px_30px_rgba(0,0,0,0.2)] border-main border`}
      >
        ...
        <div className="w-[75%] ml-9 leading-[1.6] flex flex-col justify-center gap-2">
          <h4 className="text-xl capitalize font-semibold">{brand.name}</h4>
        </div>
      </div>
    </Link>
  );
